...
class openHABSkill(MycroftSkill):
                        ...
                        if "Control" in json_response[x]['tags']:
                            ...
                            if "Light" in json_response[x]['tags']:
                                ...
                                self.switchableItemsDic.update(
                                    {json_response[x]['name']: json_response[x]['label']})
                        elif "Help" in json_response[x]['tags']:
                            self.helpItemsDic.update({json_response[x]['name']: json_response[x]['label']})
                        else:
                            # Old tags
    ...
    def handle_list_items_intent(self, message):
        ...
        msg = msg.strip() + ' ' + self.getItemsFromDict("Help Switches", self.helpItemsDic) + "\n"
    ...
    def handle_refresh_tagged_items_intent(self, message):
        ...
        self.getTaggedItems()
        dictLenght = str(len(self.lightingItemsDic) + len(self.switchableItemsDic) + len(self.helpItemsDic) + len(self.currentTempItemsDic) + len(self.currentHumItemsDic) + len(
            self.currentThermostatItemsDic) + len(
            self.targetTemperatureItemsDic) + len(self.homekitHeatingCoolingModeDic))
